...
/**
 * What one read of the source yields.
 *
...
export type PushResult = {
  event: StreamEvent;
  /**
   * One fingerprint per run (`digests.ts`) — what a tick diffs against the subscriber's last
   * map to fill `affected` (#69).
   */
  digests: Map<string, string>;
  /**
   * Every terminal handle the retained runs name — each orchestrator, and every agent it ever
   * dispatched. It is the **exact join key** the enrichment decorator matches Orca's live
   * worker list against (#61), and it rides here because the runs it is read off no longer
   * travel on the event: #69 took the full history off the wire, and the decorator would
   * otherwise have to re-read the whole database to learn a list the push just built.
   */
  handles: string[];
};
