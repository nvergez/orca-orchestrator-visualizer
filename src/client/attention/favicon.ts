 ...
 * second thing to get wrong on a `404`. A string cannot fail to load.
 *
 * They are the tab's, and the tab's only. A notification does **not** wear this mark, however much
 * it would like to: Chrome and Firefox decode a notification icon as a raster image and an SVG
 * renders as nothing at all (`notify.ts`).
 *
...
const BADGE_INK = '#ef4444';

/** The mark: one task, and the two it dispatched — the smallest true drawing of a DAG. */
const MARK = `
...
/** The one dot every unread thing on a screen has ever worn. */
const BADGE = `<circle cx="25" cy="7" r="5.6" fill="${BADGE_INK}" stroke="${TILE}" stroke-width="2"/>`;
