...
import type { TaskDetail, WorkerCompletion } from '../shared/types.ts';
...
export function archiveTaskLoader(archive: RunArchive): TaskLoader {
  return async (taskId) => {
    ...
    // The outcome receipt (#67), read offline from the two evidence sources the archive already
    // carries raw: `tasks.result` in `bodies`, and the `worker_done` rows in `messages`. Derived
    // here rather than stored in the file, for exactly the reason the attempts are joined rather
    // than duplicated — the file keeps *one* copy of the evidence, and a reading of it that lived
    // beside that copy is a reading that could disagree with it (`shared/archive.ts`).
    //
    // The payload rides as the archive's serialization of the row rather than the column text
    // byte-for-byte: the raw bytes stayed behind in the database, and this is the closest a
    // self-contained file gets to them.
    const completions: WorkerCompletion[] = archive.messages
      .filter((message) => message.type === 'worker_done' && taskIdOf(message.payload) === taskId)
      .map((message) => ({
