...
describe('the cast', () => {
  ...
  it('shows never-heartbeating health consistently in the cast and run summary', () => {
    ...
    render(<App event={event([run({ id: 'run_crew', cast: [A1, A2] })], tasks)} />);
    ...
    // #48 owns whether the orchestration itself is "live"; worker health is additive and does not
    // rewrite that server-owned semantic.
    expect(within(row('run_crew')).getByTestId('live-dot')).toHaveAttribute('data-live', 'false');
  });
