...
import type { DurationObservation, Meta, Run, StreamEvent, Task } from '../../src/shared/types.ts';
 ...
 * is tested value by value. The **presentation** is `<App>` fed a canned `StreamEvent`, asserting
...
function eventOf(tasks: Task[]): StreamEvent {
  return {
    seq: 1,
    meta: META,
...
describe('the critical path on the canvas', () => {
  it('statically highlights the path of a completed run, and says what it is', async () => {
    ...
    render(<App event={eventOf(tasks)} />);
