...
import { EventStream, type PushResult, type StreamSource } from '../../src/server/stream.ts';
...
describe('GET /api/stream', () => {
  ...
  it('cannot skip a message committed while a snapshot is choosing its cursor', async () => {
    ...
    // A first connect carries no message backfill at all (#69): a client that has seen nothing
    // has missed nothing. So the omission is total here — which makes the *cursor* the whole of
    // what #49 is about, and it is still the one thing that can go wrong.
    expect(first.event.messages).toEqual([]);
    // **The #49 invariant.** The id is read off the rows the event actually materialized (1),
    // never re-queried as `MAX(sequence)` afterwards — that query would now return 2, because
    // the concurrent commit has already landed, and the row would then sit *below* the advertised
    // cursor and be skipped for the life of the connection. One is the honest high-water mark of
    // what this read saw; two would be a promise about a row nobody was ever sent.
    expect(first.event.seq).toBe(1);
...
class CountingSource implements StreamSource {
  ...
  push(): PushResult {
    ...
    // No runs in this stub world, so nothing to fingerprint and no handle to enrich (#61).
    return { event, digests: new Map(), handles: [] };
  }
}
