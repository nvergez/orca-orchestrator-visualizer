...
async function snapshotOf(builder: FixtureBuilder): Promise<SnapshotView> {
  ...
  // `since: 0` — "everything after cursor 0", the whole feed. A *first* connect has carried no
  // message delta since #69 (a client that has seen nothing has missed nothing), and this suite
  // asks what a missing column costs the messages, so it has to ask for the rows.
