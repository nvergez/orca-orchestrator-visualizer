...
describe('the boundary: what an archive is forbidden to carry', () => {
  ...
  it('exports no liveness claim: a run that is live *now* is archived as ended', async () => {
    const dbPath = twoOrchestrators()
      // A run is `live` when Orca is up **and** the run is `active` — recent evidence, not merely
      // an unconverged one (#81, SPEC §12.3). The rest of this fixture is anchored days in the
      // past, where the honest health is `silent`; so the thing this test needs — a run that is
      // running *right now* — is spelled out as what that actually means: a worker beating a
      // moment ago.
      .dispatch({
        taskId: A2,
        assigneeHandle: AGENT,
        status: 'dispatched',
        dispatchedAt: new Date(Date.now() - 2 * MINUTE),
    ...
    // The live view says this run is running: Orca is up, and A2 is dispatched and beating.
